...
!(function() {
  ...
  var b = [],
    ...
    e = function(a, b) {
      return (
        (a = a.trim().toLowerCase()), (b = b.trim().toLowerCase()), a === b ? 0 : a < b ? 1 : -1
      );
    },
    f = function(a, b) {
      return function(c, d) {
            ...
            if (!(h.table.tBodies[m].rows.length < 2)) {
              for (p = 0; p < h.table.tBodies[m].rows.length; p++)
                (k = h.table.tBodies[m].rows[p]),
                  ...
                  s++;
              for (
                'descending' === o ? q.sort(f(j, !0)) : (q.sort(f(j, !1)), q.reverse()), p = 0;
                p < s;
                p++
              )
                r[p] ? ((k = r[p]), t++) : (k = q[p - t].tr), h.table.tBodies[m].appendChild(k);
            }
          }
